...
CHART="$ROOT/charts/hydradb"

usage() {
  cat <<'EOF'
Deploy a public, S3-backed HydraDB instance to an existing single-node K3s cluster.

Required:
  HYDRADB_S3_BUCKET       S3 bucket used as durable storage.

Usually automatic on EC2, otherwise required:
  HYDRADB_PUBLIC_HOST     Public DNS name or IPv4 address used by clients.

Common optional settings:
  HYDRADB_AWS_REGION              Default: AWS config region or us-east-1
  HYDRADB_K8S_NAMESPACE           Default: hydradb
  HYDRADB_RELEASE                 Default: hydradb
  HYDRADB_GRAPH_NAMESPACE         Default: single-node
  HYDRADB_GRAPH_ID                Default: public-test
  HYDRADB_DATABASE                Default: default
  HYDRADB_S3_PREFIX               Default: hydradb/<graph namespace>
  HYDRADB_BOLT_NODE_PORT          Default: 30687
  HYDRADB_HTTPS_NODE_PORT         Default: 30443
  HYDRADB_AUTH_TOKEN              Optional token of at least 32 characters
  HYDRADB_AWS_SECRET_NAME         Default: <release>-aws-credentials
  HYDRADB_USE_INSTANCE_ROLE       Set to 1 to avoid a static AWS Secret
  HYDRADB_SKIP_BUILD              Set to 1 to reuse the imported image
  HYDRADB_ROTATE_SECRETS          Set to 1 to rotate auth and TLS Secrets
  HYDRADB_STATE_DIR               Default: ~/.config/hydradb-single-node
...
Examples:
  ...
  HYDRADB_S3_BUCKET=graph-benchmark HYDRADB_PUBLIC_HOST=203.0.113.10 \
    ./scripts/deploy_single_node_k3s.sh

Validation without changing a cluster:
  HYDRADB_S3_BUCKET=ci-bucket HYDRADB_PUBLIC_HOST=203.0.113.10 \
...
S3_BUCKET="${HYDRADB_S3_BUCKET:-}"
AWS_REGION_VALUE="${HYDRADB_AWS_REGION:-}"
K8S_NAMESPACE="${HYDRADB_K8S_NAMESPACE:-hydradb}"
RELEASE="${HYDRADB_RELEASE:-hydradb}"
GRAPH_NAMESPACE="${HYDRADB_GRAPH_NAMESPACE:-single-node}"
GRAPH_ID="${HYDRADB_GRAPH_ID:-public-test}"
DATABASE="${HYDRADB_DATABASE:-default}"
S3_PREFIX="${HYDRADB_S3_PREFIX:-hydradb/$GRAPH_NAMESPACE}"
BOLT_NODE_PORT="${HYDRADB_BOLT_NODE_PORT:-30687}"
HTTPS_NODE_PORT="${HYDRADB_HTTPS_NODE_PORT:-30443}"
PUBLIC_HOST="${HYDRADB_PUBLIC_HOST:-}"
PUBLIC_IP="${HYDRADB_PUBLIC_IP:-}"
ROTATE_SECRETS="${HYDRADB_ROTATE_SECRETS:-0}"
SKIP_BUILD="${HYDRADB_SKIP_BUILD:-0}"
USE_INSTANCE_ROLE="${HYDRADB_USE_INSTANCE_ROLE:-0}"
STATE_DIR="${HYDRADB_STATE_DIR:-$HOME/.config/hydradb-single-node}"
IMAGE_REPOSITORY="${HYDRADB_IMAGE_REPOSITORY:-docker.io/library/hydradb}"
IMAGE_TAG="${HYDRADB_IMAGE_TAG:-single-node}"
IMAGE="$IMAGE_REPOSITORY:$IMAGE_TAG"
DEPLOYMENT_NONCE="${HYDRADB_DEPLOYMENT_NONCE:-$(date +%s)}"
AWS_SECRET_NAME="${HYDRADB_AWS_SECRET_NAME:-$RELEASE-aws-credentials}"
AUTH_SECRET_NAME="${HYDRADB_AUTH_SECRET_NAME:-$RELEASE-auth}"
PUBLIC_TLS_SECRET_NAME="${HYDRADB_PUBLIC_TLS_SECRET_NAME:-$RELEASE-public-tls}"

[[ -n "$S3_BUCKET" ]] || die "HYDRADB_S3_BUCKET is required"
[[ "$S3_BUCKET" =~ ^[a-z0-9][a-z0-9.-]{1,61}[a-z0-9]$ ]] || die "invalid S3 bucket name"
validate_kubernetes_name "$K8S_NAMESPACE" || die "invalid HYDRADB_K8S_NAMESPACE"
validate_kubernetes_name "$RELEASE" || die "invalid HYDRADB_RELEASE"
validate_graph_component "$GRAPH_NAMESPACE" || die "invalid HYDRADB_GRAPH_NAMESPACE"
validate_graph_component "$GRAPH_ID" || die "invalid HYDRADB_GRAPH_ID"
validate_graph_component "$DATABASE" || die "invalid HYDRADB_DATABASE"
[[ "$S3_PREFIX" =~ ^[A-Za-z0-9][A-Za-z0-9._/-]*[A-Za-z0-9]$ ]] || die "invalid HYDRADB_S3_PREFIX"
[[ "$S3_PREFIX" != *'..'* ]] || die "HYDRADB_S3_PREFIX must not contain '..'"
validate_node_port "$BOLT_NODE_PORT" || die "HYDRADB_BOLT_NODE_PORT must be in 30000-32767"
validate_node_port "$HTTPS_NODE_PORT" || die "HYDRADB_HTTPS_NODE_PORT must be in 30000-32767"
[[ "$BOLT_NODE_PORT" != "$HTTPS_NODE_PORT" ]] || die "Bolt and HTTPS NodePorts must differ"
[[ "$DEPLOYMENT_NONCE" =~ ^[A-Za-z0-9._-]+$ ]] || die "invalid HYDRADB_DEPLOYMENT_NONCE"
...
[[ "$AWS_REGION_VALUE" =~ ^[a-z0-9-]+$ ]] || die "invalid HYDRADB_AWS_REGION"
...
[[ -n "$PUBLIC_HOST" ]] || die "set HYDRADB_PUBLIC_HOST; EC2 public metadata was unavailable"
validate_dns_or_ipv4 "$PUBLIC_HOST" || die "invalid HYDRADB_PUBLIC_HOST"
if [[ -n "$PUBLIC_IP" ]]; then
  is_ipv4 "$PUBLIC_IP" || die "invalid HYDRADB_PUBLIC_IP"
  validate_dns_or_ipv4 "$PUBLIC_IP" || die "invalid HYDRADB_PUBLIC_IP"
fi
...
extraObjects:
  - apiVersion: v1
    kind: Service
    metadata:
      name: '{{ include "hydradb.fullname" . }}-public-bolt'
      labels:
  ...
  - apiVersion: v1
    kind: Service
    metadata:
      name: '{{ include "hydradb.fullname" . }}-public-https'
      labels:
        app.kubernetes.io/name: '{{ include "hydradb.name" . }}'
        app.kubernetes.io/instance: '{{ .Release.Name }}'
        app.kubernetes.io/component: node
...
if [[ "$USE_INSTANCE_ROLE" != "1" ]]; then
  ...
  if [[ -n "$ACCESS_KEY_VALUE" && -n "$SECRET_KEY_VALUE" ]]; then
    ...
    die "AWS credentials are unavailable; configure AWS CLI, set AWS environment variables, pre-create $AWS_SECRET_NAME, or set HYDRADB_USE_INSTANCE_ROLE=1"
  fi
fi
...
if secret_exists "$AUTH_SECRET_NAME" && [[ "$ROTATE_SECRETS" != "1" ]]; then
  ...
  AUTH_TOKEN="${HYDRADB_AUTH_TOKEN:-$(openssl rand -hex 32)}"
...
if secret_exists "$PUBLIC_TLS_SECRET_NAME" && [[ "$ROTATE_SECRETS" != "1" ]]; then
  ...
  certificate_matches_host "$PKI_DIR/public-existing.crt" "$PUBLIC_HOST" || \
    die "existing public certificate does not cover $PUBLIC_HOST; set HYDRADB_ROTATE_SECRETS=1"
else
