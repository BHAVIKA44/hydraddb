...
IMAGE="${GRAPH_BENCH_IMAGE:-hydradb-graphblas-benchmark:local}"
TOKEN="s3-bolt-benchmark-secret-32-chars"
RUN_ID="$(date -u +%Y%m%dT%H%M%SZ)-$$"
PREFIX="${GRAPH_BENCH_PREFIX:-hydradb/benchmarks/graphblas-degree-${DEGREE}/${RUN_ID}}"
RESULT_DIR="${GRAPH_BENCH_RESULT_DIR:-$ROOT_DIR/bench-results/ec2-graphblas-${RUN_ID}}"
VENV="${GRAPH_BENCH_VENV:-$HOME/.cache/hydradb-graphblas-benchmark-venv}"
RESULT_DIR="$(realpath -m "$RESULT_DIR")"
VENV="$(realpath -m "$VENV")"
CONTAINER="hydradb-graphblas-bench-${RUN_ID,,}"
...
case "$PREFIX" in
  hydradb/benchmarks/*) ;;
  *) echo "GRAPH_BENCH_PREFIX must remain under hydradb/benchmarks/" >&2; exit 2 ;;
esac
